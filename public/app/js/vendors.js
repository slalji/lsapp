 ...
 jQuery(function($) {
    ...
    var myTable =
        $('#dynamic-table').DataTable( {
            ...
            bAutoWidth: false,
            ajax: {
                url: "ajax/vendorServerSide.php", // json datasource
                data: {vendor:$('#vendor').val(),month:$('#month').val()},
    ...
    $("div.toolbar").html('<div class="dataTables_length"></div><div><a href="#" id="advsearch" data-toggle="modal" data-target="#myModal" class="btn btn-info">Compare</a><div>');
    /* Apply the tooltips */
       ...
       myTable.columns(0).search(param).draw();
        ...
        if ($('#isdownload input:radio:checked').val() == 'yes'){
            ...
            $.ajax({
             ...
             type: "post",
             "data": {fulltimestamp:$('#date-text').html(), vendor:$('#vendor').val(), download:'yes' },
             beforeSend: function() {
                // setting a timeout
       ...
       $.ajax({
        ...
        if(xhr != undefined){
            ...
            //console.log(datasets);
                    ...
                    data1.push(datasets['vendor']);
                    data1.push(datasets['compare']);

                    if (data1.length > 0) {
                        $.plot("#placeholder", data1, {
                            
                            series: {
                                xcolor: ["#3fcf7f","#ffcf79"],
                                lines: {
                                    show: true, 
                                    fill:true,
